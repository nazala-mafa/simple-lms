  ...
  <div class="container">
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h2>Edit "{{ $school->name }}" School</h2>
        <a class="btn btn-primary" href="{{ route('master.school.status.index') }}">Add school status<a>
      </div>
      <div class="card-body">
        <form method="POST" action="{{ route('master.school.update', $school->id) }}">
          @csrf
          @method('PATCH')

          <div class="mb-3">
            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
              value="{{ old('name', $school->name) }}" placeholder="School Name">
            @error('name')
          ...
          <div class="mb-3">
            <input type="email" name="email" class="form-control @error('email') is-invalid @enderror"
              value="{{ old('email', $school->email) }}" placeholder="School Email">
            @error('email')
              <div class="invalid-feedback">
                {{ $message }}
              </div>
